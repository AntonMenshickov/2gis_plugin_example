...
internal class GisMapView(
    ...
    private var mapObjectManagerList: MutableMap<String?, MapObjectManager> = mutableMapOf()
    ...
    override fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
        when (call.method) {
            ...
            "updateMarkers" -> {
                ...
                val iconSet: Map<String, Image> = buildMap {
                ...
                gisView.getMapAsync { map ->
                    markers.forEach {
                        if (!mapObjectManagerList.containsKey(it["layer"])) {
                            mapObjectManagerList[it["layer"] as String?] =
                                MapObjectManager.withClustering(
                                    map,
                                    logicalPixel = LogicalPixel(16.0f),
                                    maxZoom = Zoom(16.0f),
                                    clusterRenderer = MapClusterRenderer()
                                );
                        }
                    }
                    val groupedObjects: Map<String?, MutableList<Map<String, Any>>> = buildMap {
                        markers.forEach {
                            if (!this.containsKey(it["layer"])) {
                                this[it["layer"] as String?] = mutableListOf<Map<String, Any>>()
                            }
                            this[it["layer"] as String?]!!.add(it)
                        }
                    }
                    groupedObjects.entries.forEach {
                        controller.updateMarkers(
                            it.value,
                            iconSet,
                            mapObjectManagerList[it.key]!!
                        )
                    }
                }
            }
